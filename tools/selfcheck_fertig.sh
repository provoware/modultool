#!/bin/bash
# tools/selfcheck_fertig.sh – Automatischer System- und Dateicheck für Modultool
...
bad_linebreaks=$(grep -Il $'\r' tools/selfcheck_fertig.sh)
if [ -n "$bad_linebreaks" ]; then
  echo "❌ Fehler: Datei enthält Windows-Zeilenumbrüche (CRLF): $bad_linebreaks"
  echo "💡 Tipp: dos2unix tools/selfcheck_fertig.sh"
  exit 1
fi
